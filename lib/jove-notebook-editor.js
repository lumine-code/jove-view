/**
 * JoveNotebookEditor - Editor for Jupyter notebooks using React
 */

const { Emitter, CompositeDisposable, Disposable, TextBuffer } = require("atom");
const path = require("path");

const HISTORY_METADATA_KEY = "jove_view";
...
/**
 * JoveNotebookEditor is a view/editor for a NotebookDocument.
 * Multiple editors can share the same document (like Lumine's TextEditor/TextBuffer).
 */
class JoveNotebookEditor {
  static deserialize(state, deserializeServices = null) {
    ...
    for (const paneContainer of [
      ...
      for (const pane of paneContainer.getPanes()) {
        for (const item of pane.getItems()) {
          if (item instanceof JoveNotebookEditor && item.view && !item._destroyed) {
    ...
    const editor = new JoveNotebookEditor(null, state, deserializeServices);
    return editor;
  }
  ...
  constructor(notebookDocument, deserializeState = null, deserializeServices = null) {
    ...
    this._containerElement.className = "jove-view jove-notebook-container";
    this._containerElement._joveNotebookEditor = this;
  ...
  _createPlaceholder() {
    ...
    this._containerElement.innerHTML =
      '<div class="jove-notebook-loading"><div class="loading-spinner-large"></div><div class="loading-message">Loading notebook...</div></div>';
  }
  ...
  async _loadFromState(state) {
    ...
    this._loadingPromise = (async () => {
      try {
        ...
        console.error("[jove-view] Failed to load notebook:", error);
  ...
  _initWithDocument(notebookDocument) {
    ...
    atom.packages.triggerActivationHook?.("jove.adapter:item-used");
        ...
        if (item === this && this.view) {
          requestAnimationFrame(() => {
            if (this.sourceEditor) {
              this.sourceEditor.joveNotebookEditor = this;
            }
  ...
  async _setupSourceEditor() {
    if (!this.document._sourceEditor) {
      ...
      editor.isJoveNotebookSourceEditor = true;
      editor.joveNotebookEditor = this;
  ...
  async createSourceEditor(serializedState = null) {
    ...
    if (serializedState?.bufferState && typeof TextBuffer?.deserialize === "function") {
      try {
        ...
        console.warn("[jove-view] Could not deserialize source buffer:", error.message);
      }
    }
  ...
  triggerSourceEditorActivationHooks() {
    ...
    atom.packages.triggerActivationHook?.("jove-view:grammar-used");
  }
  ...
  parseSourceEditorSnapshot(text) {
    try {
      ...
      console.warn("[jove-view] Could not apply source editor snapshot:", error.message);
      return null;
    }
  }
    ...
    requestAnimationFrame(() => {
      for (const entry of cursorPlan) {
        ...
        try {
          ...
          console.warn("[jove-view] Could not restore source history cursor:", error.message);
        }
      }
  ...
  copy() {
    const copy = new JoveNotebookEditor(this.document);
    ...
    if (cells && cells[this.activeCellIndex] && this._pendingCursorPositions) {
      ...
      if (activeCellView && activeCellView.editor && cursorState) {
        try {
          ...
          console.warn("[jove-view] Could not restore cursor position:", e.message);
        }
      }
    ...
    if (this.document.filePath) {
      ...
      if (this.document.isModified()) {
        return this.withSerializedSourceEditor(
          {
            deserializer: "JoveNotebookEditor",
  ...
  serializeSourceEditorState() {
    ...
    if (typeof buffer?.serialize === "function") {
      try {
        ...
        console.warn("[jove-view] Could not serialize source buffer:", error.message);
      }
    }
    ...
    if (typeof this.sourceEditor.serialize === "function") {
      try {
        ...
        console.warn("[jove-view] Could not serialize source editor:", error.message);
      }
    }
...
module.exports = JoveNotebookEditor;
