...
class CellView {
  ...
  applyOutputHeight() {
    ...
    // The output view renders this height itself, so setting the style here
    // would only be undone by its next render.
    this.outputView.update({ maxHeight: atom.config.get("jupyter-view.output.maxHeight") });
  }
