...
function getNotebookEditorClass() {
  return require("./jove-notebook-editor");
}

function isNotebookEditor(item) {
  const JoveNotebookEditor = getNotebookEditorClass();
  return item instanceof JoveNotebookEditor || item?.constructor?.name === "JoveNotebookEditor";
}
...
class JoveAdapter {
  ...
  getAdapterId() {
    ...
    if (!this.editor._joveAdapterId) {
      this.editor._joveAdapterId = `jove-view:${Date.now()}:${Math.random().toString(36).slice(2)}`;
    }
    return this.editor._joveAdapterId;
  }
...
module.exports = JoveAdapterService;
