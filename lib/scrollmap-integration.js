...
class NotebookScrollmap {
  constructor(editor, Simplemap) {
    ...
    this.disposables.add(
      ...
      atom.config.onDidChange("jove-view.scrollmap.enabled", () => this.scheduleUpdate()),
      atom.config.onDidChange("jove-view.scrollmap.threshold", () => this.scheduleUpdate()),
      atom.config.onDidChange("jove-view.scrollmap.depth", () => this.scheduleUpdate()),
      atom.config.onDidChange("jove-view.input.maxHeight", () => this.scheduleUpdate()),
      atom.config.onDidChange("jove-view.output.maxHeight", () => this.scheduleUpdate()),
  ...
  insertScrollmap() {
    ...
    if (!this.scrollmap) {
      ...
      this.scrollmap.element.classList.add("jove-header-map");
    }
  ...
  isEnabled() {
    return atom.config.get("jove-view.scrollmap.enabled") !== false;
  }

  getThreshold() {
    return atom.config.get("jove-view.scrollmap.threshold") || 0;
  }

  getDepth() {
    return atom.config.get("jove-view.scrollmap.depth") || 0;
  }
  ...
  positionMap(view, container) {
    const toolbar = view.element.querySelector(".jove-notebook-toolbar");
  ...
  cellMarkers(container) {
    ...
    return markedCells.map(({ index, active }) => {
      ...
      return {
        ...
        cls: active ? `marker-jove-active marker-jove-active-${mode}` : "marker-jove-selected",
      };
    });
  }
